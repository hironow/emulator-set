...
def test_firebase_container_starts():
    ...
    try:
        ...
        assert container.status == 'running', f"Firebase container is not running, status: {container.status}"
    except docker.errors.NotFound:
        pytest.fail("Firebase container 'firebase-emulator' not found. Run 'docker compose up firebase-emulator' first.")
