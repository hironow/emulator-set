...
def test_github_actions_workflow_runs_pytest():
    ...
    has_pytest = any(
        'pytest' in str(step.get('run', '')) and ('test_' in str(step.get('run', '')) or 'tests/' in str(step.get('run', '')))
        for step in steps
    )
...
def test_github_actions_uses_latest_versions():
    ...
    checkout_step = next((step for step in steps if step.get('uses', '').startswith('actions/checkout@')), None)
    ...
    python_step = next((step for step in steps if step.get('uses', '').startswith('actions/setup-python@')), None)
    assert python_step is not None, "Workflow must use actions/setup-python"
    assert python_step['uses'] == 'actions/setup-python@v5', "Should use actions/setup-python@v5 (latest)"