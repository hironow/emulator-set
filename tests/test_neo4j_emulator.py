...
def test_neo4j_container_starts():
    ...
    try:
        ...
        assert container.status == 'running', f"Neo4j container is not running, status: {container.status}"
    except docker.errors.NotFound:
        pytest.fail("Neo4j container 'neo4j-emulator' not found. Run 'docker compose up neo4j' first.")
