...
func main() {
    ...
    for {
        if len(multiline) == 0 { fmt.Print("postgres> ") } else { fmt.Print("      -> ") }
...
func executeQuery(db *sql.DB, query string) {
    ...
    if strings.HasPrefix(upper, "SELECT") || strings.HasPrefix(upper, "WITH") || strings.HasPrefix(upper, "SHOW") {
        ...
        for rows.Next() {
            if err := rows.Scan(ptrs...); err != nil { fmt.Printf("❌ %v\n", err); return }
            out := make([]string, len(cols))
            for i, v := range vals {
                if v == nil { out[i] = "NULL" } else { out[i] = fmt.Sprintf("%v", v) }
            }
            table.Append(out)
        }
        table.Render()
    ...
    res, err := db.Exec(q)
    if err != nil { fmt.Printf("❌ %v\n", err); return }
    if n, err := res.RowsAffected(); err == nil { fmt.Printf("✅ %d rows affected\n", n) } else { fmt.Println("✅ OK") }
}
